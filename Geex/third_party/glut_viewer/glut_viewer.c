...
void glut_viewer_set_zoom(float scale)  {
	zoom = scale ;
}
float glut_viewer_get_zoom() {
	return zoom ; 
}
...
void glut_viewer_save_view_params(const char* filename) {
    FILE* out ;
	int i ;
    out = fopen(filename, "wb") ;
	fprintf(out, "%d %d\n", glut_viewer_W, glut_viewer_H) ;
	for(i=0; i<4; ++i) fprintf(out, "%f ", cur_rot[i]) ;
	fprintf(out, "\n") ;
	for(i=0; i<4; ++i) fprintf(out, "%f ", cur_rot_light[i]) ;
	fprintf(out, "\n") ;
	fprintf(out, "%f", zoom) ;
	fprintf(out, "\n") ;
	for(i=0; i<3; ++i) fprintf(out, "%f ", cur_xlat[i]) ;
	fprintf(out, "\n") ;
	fprintf(out, "%f %f %f %f %f %f", xmin, ymin, zmin, xmax, ymax, zmax) ;
	fprintf(out, "\n") ;
	fprintf(out, "%f", roi_radius) ;
	fprintf(out, "\n") ;
	fclose(out) ;
}

void glut_viewer_load_view_params(const char* filename) {
    FILE* in ;
	int w, h ;
	int i ;
	...
	fscanf(in, "%d%d", &w, &h) ;
	for(i=0; i<4; ++i) fscanf(in, "%f", &cur_rot[i]) ;
	for(i=0; i<4; ++i) fscanf(in, "%f", &cur_rot_light[i]) ;
	fscanf(in, "%f", &zoom) ;
	for(i=0; i<3; ++i) fscanf(in, "%f", &cur_xlat[i]) ;
	fscanf(in, "%f %f %f %f %f %f", &xmin, &ymin, &zmin, &xmax, &ymax, &zmax) ;
	fscanf(in, "%f", &roi_radius) ;
	fclose(in) ;
	reshape(w, h) ;
	fclose(in) ;
}
