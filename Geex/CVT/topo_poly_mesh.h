...
namespace Geex {
    ...
    public:
		...
		TopoPolyFacetInfo() : flags(0), id(~0), id_before_partition(~0) { } //dxy
        TopoFlags flags ;
        unsigned int id ;
		//dxy add
		unsigned int id_before_partition;
		//
    } ;
    ...
    public:
		...
		void facet_centroid_and_mass_w(unsigned int f, vec3& g, double& V) {
			g = vec3(0, 0, 0) ;
			V = 0 ;
			unsigned int i0 = facet_begin(f) ;
			vec3 v0 = vertex(i0) ;
			for(unsigned int i = facet_begin(f)+1; i+1<facet_end(f); i++) {
				...
				vec3   gi ;
				double Vi ;
				tri_centroid(v0, v1, v2, vertex(i0).w, vertex(i).w, vertex(i+1).w, gi, Vi) ;
				V += Vi ; //area ;
				g += gi ; // (area/3.0)*(vertex(i)+vertex(i+1)+vertex(i0)) ;
			}
		}
...
//                      << " = " << I.sym << std::endl ;
        ...
        const std::vector<unsigned int>& vertex_index() const {
		...
		///dxy add
		std::vector<vec3>& vertex_field() {
			return vertex_field_;
		}

		const std::vector<vec3>& vertex_field() const {
			return vertex_field_;
		}
		...
		const std::vector<vec3>& vertex_normal() const {
			return vertex_normal_;
		}

		std::vector<vec3>& facet_field() {
			return facet_field_;
		}

		const std::vector<vec3>& facet_field() const {
			return facet_field_;
		}

		std::vector<vec3>& facet_normal() {
			return facet_normal_;
		}

		const std::vector<vec3>& facet_normal() const {
			return facet_normal_;
		}

		/** access field direction via vertex index
		*/
		vec3 vertex_field(unsigned int i) const {
			gx_assert(i < vertex_field_.size());
			return vertex_field_[i];
		}

		vec3 vertex_field(unsigned int i) {
			gx_assert(i < vertex_field_.size());
			return vertex_field_[i];
		}

		/** access vertex normal direction via vertex index
		*/
		vec3 vertex_normal(unsigned int i) const {
			gx_assert(i < vertex_normal_.size());
			return vertex_normal_[i];
		}

		vec3 vertex_normal(unsigned int i) {
			gx_assert(i < vertex_normal_.size());
			return vertex_normal_[i];
		}

		/** access facet field direction via facet index
		*/
		const vec3 &facet_field(unsigned int i) const {
			gx_assert(i < facet_field_.size());
			return facet_field_[i];
		}

		vec3 &facet_field(unsigned int i) {
			gx_assert(i < facet_field_.size());
			return facet_field_[i];
		}

		/** access facet normal direction via facet index
		*/
// 		const vec3 &facet_normal(unsigned int i) const {
// 			gx_assert(i < facet_normal_.size());
// 			return facet_normal_[i];
// 		}
// 
// 		vec3 &facet_normal(unsigned int i) {
// 			gx_assert(i < facet_normal_.size());
// 			return facet_normal_[i];
// 		}
		///
    ...
    private:
        ...
        bool orientation_ ;
		///dxy add
		std::vector<vec3> vertex_field_;
		std::vector<vec3> vertex_normal_;
		std::vector<vec3> facet_field_;
		std::vector<vec3> facet_normal_;
		///
