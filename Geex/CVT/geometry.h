...
namespace Geex {
	...
	template <class T> T tri_circumradius(
		const vec3g<T>& p, const vec3g<T>& q, const vec3g<T>& r) {
		const vec3g<T> c = tri_circumcenter(p, q, r);
		return distance(c, p) ;
	}
	...
	template <class T> inline vec3g<T> random_point_in_tetra(
		const vec3g<T>& p1, const vec3g<T>& p2, const vec3g<T>& p3, const vec3g<T>& p4)
	{
        ...
        return vec3g<T>(a*p1 + s*p2 + t*p3 + u*p4) ;
	}
	...
	template <class T> inline T to_arc(const T ang) {
		return ang/(T)180 * M_PI ;
	}

	template <class T> inline T to_ang(const T arc) {
		return arc/M_PI*(T)180 ;
	}
