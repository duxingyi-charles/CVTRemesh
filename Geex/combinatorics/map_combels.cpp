...
//_________________________________________________________
    ...
    namespace MapCombels {
		...
		//dxy add:
		bool Vertex::is_sharp(double dihedral_dot) const {
			Halfedge *it = halfedge();
			do {
				if(it->is_sharp(dihedral_dot)) {
					return true;
				}
				it = it->next_around_vertex();
			} while (it != halfedge());
		...
		bool Vertex::is_corner(double dihedral_dot) const {
			int sharp_count = 0;
			Halfedge *it = halfedge();
			do {
				if(it->is_sharp(dihedral_dot)) {
		...
		vec3 Facet::normal() const {
			//dxy add: degenerate facet
			if (halfedge() == nil)
			{
				return vec3(0, 0, 0);
			}
			else if (halfedge()->next()->vertex() == halfedge()->prev()->vertex())
			{
				return vec3(0, 0, 0);
			}
			//
