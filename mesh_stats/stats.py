...
def mesh_stats(filename):
	...
	num_obtuse = 0
	num_lt_thirty = 0
	edges = {}  # dict: (v1,v2) -> set of face index
	...
	for f in face:
		...
		for i in xrange(3):
			...
			cur_e = (vid1, vid2)
			if cur_e in edges:
				edges[cur_e].add(fidx)
			else:
				edges[cur_e] = set([fidx])
		# Vertex Neighbor
	...
	minA = min_angle.min()
	maxA = max_angle.max()
	argmin_A = min_angle.argmin()
	argmax_A = max_angle.argmax()
	...
	# find boundary/non-manifold edges
	boundary = []
	non_manifold = []
	edges_fidx = edges.items()
	for e, fidx in edges_fidx:
		if len(fidx) == 1:
			boundary.append(e)
		elif len(fidx) > 2:
			non_manifold.append(e)
	if len(non_manifold) > 0:
		print("Warning! found %d non-manifold edges:" % (len(non_manifold)))
		print(non_manifold)
	# mark boundary vertices
	on_boundary = [False] * len(vert)
	for e in boundary:
	...
	is_regular = np.logical_or(np.logical_and(on_boundary, valence==4),
								np.logical_and(np.logical_not(on_boundary), valence==6))
	regular_rate = np.sum(is_regular) * 100.0 / len(vert)
	#test
	print(hist)
	...
	with open(filename, "w") as f:
		f.write("|V|, %-5i, |F|, %-5i, |t_obt|, %-5i, minQ, %-6f, avgQ, %-6f, minS, %-6f, maxS, %-6f, stdS, %-6f, minA, %-6f, maxA, %-6f, elemMinA, %-6f, elemMaxA, %-6f, stdA, %-6f, <30, %-6f, >90, %-6f, V567, %-6f , v6, %-6f , regular, %-6f , "
		% (len(vert), len(face), num_obtuse, minQ, avgQ, minS, maxS, stdS, minA, maxA, elem_avg_minA, elem_avg_maxA, stdA, lt_thirty_rate, obtuse_rate, v567_rate, v6_rate, regular_rate))
		...
		f.write("min angle face id, %i, max angle face id, %i, " % (argmax_A, argmax_A))
	return True
